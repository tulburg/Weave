...
export default class Socket {
      ...
      Object.keys(this.registry).forEach(event => {
        socket.on(event, data => {
          const instance: SocketInstance = {
            io: options.io, socket, event, store: {}
          } as any;
  ...
  map = (type: 'headers' | 'body', keys: [string], success: CalleeFunction, fail: CalleeFunction) => {
    const types = {
      headers: 'header',
      body: 'body'
    };
    const fn = (_: any, instance: any) => {
      if (!instance['private' + type]) return { code: 400, message: 'Invalid request' };
      let checks = 0;
      keys.forEach(k => {
  ...
  mapHeader = (keys: [string], success: CalleeFunction, fail: CalleeFunction) => this.map('headers', keys, success, fail);
